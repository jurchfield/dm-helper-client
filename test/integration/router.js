...
describe('routing tests', function() {
  ...
  it('the page selector switches pages', async function() {
    ...
    await page.waitForSelector('dm-app', {visible: true});
...
async function testNavigation(page, href, linkText) {
  ...
  const dmApp = await page.$('dm-app');
  const myText = await page.evaluate(getShadowRootChildProp, dmApp, selector, 'textContent');
  expect(await myText).equal(linkText);

  // Does the click take you to the right page?
  await page.evaluate(doShadowRootClick, dmApp, selector);
...
async function testNavigationInADifferentWay(page, href, linkText) {
  const query = `dm-app::shadow a[href="/${href}"]`;
